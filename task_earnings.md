...
- **한국(11, ₩)**: 005930.KS(삼성전자)·000660.KS(SK하이닉스)·034020.KS(두산에너빌리티)·011070.KS(LG이노텍)·003550.KS(LG)·373220.KS(LG에너지솔루션)·035420.KS(NAVER)·035720.KS(카카오)·000720.KS(현대건설)·034730.KS(SK)·096770.KS(SK이노베이션).
  ...
  - 📏 **정합성 자가검증(분기 매출 앵커 범위, 이 밖이면 스케일 오류이니 재확인):** 삼성전자 **70~85조**, SK하이닉스 **12~22조**, SK이노베이션 **15~22조**, SK(지주) **28~40조**, LG에너지솔루션 **5~8조**, 현대건설 **7~9조**, LG이노텍 **4~6조**, 두산에너빌리티 **4~5조**, NAVER **2.5~3조**, 카카오 **1.8~2.3조**, LG(지주) **1.8~2.3조**. **영업이익은 매출의 대개 3~15%**(예 삼성 분기 영업익 6~12조) — 매출과 비슷하거나 크면 오류다.
...
## ✅ 완료 판정 (반드시 확인)
- **`to_translate.json` 이 `[]` (빈 배열)이 될 때까지** 루프를 멈추지 마라.
- `merge_news.py` 출력의 **"번역 대기 N건"의 N 이 0** 이 되면 끝난 것이다.
- 아직 N > 0 이면 루프(① to_translate 읽기 → 40개 번역 → merge 실행)를 다시 돈다.

---

# 마무리

- **asia-news**(`grok-news.json`)를 했다면 그것도 저장한다.
- **tech-deals**(`grok-tech-news.json`)를 했다면 저장 후 `python3 merge_news.py` 를 실행한다.
- **translate**는 위 루프가 `to_translate.json` = `[]` 가 되면 완료다.
- 모든 게 끝나면 브라우저 **http://localhost:8001**
  (같은 망에서는 `http://192.168.0.16:8001`) 를 새로고침한다.

> asia-news·translate는 둘 다 권장하지만 하나만 해도 된다. 번역만 하려면 translate 루프만 돌리면 된다.
